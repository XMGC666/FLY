...
function ssr(){
    ...
    docker run -d --name=ssrmu -e NODE_ID=$port -e API_INTERFACE=glzjinmod -e MYSQL_HOST=ssr0.52node.xyz -e MYSQL_USER=span -e MYSQL_DB=span -e MYSQL_PASS=Lh7rFrLFirysbKxJ --network=host --log-opt max-size=50m --log-opt max-file=3 --restart=always stone0906/ssrmuv2
}
